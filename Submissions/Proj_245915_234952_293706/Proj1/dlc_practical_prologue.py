...
def mnist_to_pairs(nb, input, target):
    ...
    target = torch.stack(((target[a[:, 0]] > target[a[:, 1]]).float(), (target[a[:, 0]] <= target[a[:, 1]]).float()), 1)
    #target = (target[a[:, 0]] <= target[a[:, 1]])
...
def generate_pair_sets(nb):
    ...
    test_target = Variable(test_set.test_labels)
...
#    x_train, y_train, clas_train, x_test, y_test, clas_test
